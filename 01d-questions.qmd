...
The discipline of statistics is about turning data into information in order to address some research objective or question.  While there may be no such thing as a stupid question, there are ill-posed questions --- those that cannot be answered as stated.  Consider the [Deepwater Horizon Case Study](01c-casedeepwater.qmd) (@sec-caseDeepwater).  It might seem natural to ask "if a volunteer cleans wildlife, will she develop adverse respiratory symptoms?"  Let's consider the data.  Of the 54 volunteers assigned to wildlife cleaning and rehabilitation, 15 reported experiencing adverse respiratory symptoms ("nose irritation, sinus problems, or sore throat"); while some volunteers developed symptoms, others did not.  It seems the answer to our question is then "it depends" or "maybe."  This is an example of an _ill-posed question_.  Ill-posed questions are troublesome because of __variability__, the fact that every subject in the population does not behave in exactly the same way; that is, the value of a variable potentially differs from one observational unit to the next.  In our example not every volunteer had the same reaction when directly exposed to oil.

It is variability that creates a need for the discipline of statistics; in fact, you could think of statistics as the study and characterization of variability.  We must therefore learn to ask the _right_ questions --- those that can be answered in the presence of variability.
...
:::{.callout-tip}
## Big Idea
The presence of variability makes some questions ill-posed; statistics is the discipline of using data into information in the presence of variability.
:::


## Characterizing a Variable
Recall that the goal of statistical inference is to say something about the population; as a result, our research objectives should be about the population.  The first step to constructing a well-posed question is then to identify the population of interest for the study.  For the [Deepwater Horizon Case Study](01c-casedeepwater.qmd), it is unlikely that we are only interested in these 54 observed volunteers assigned to wildlife cleaning.  In reality, we probably want to say something about volunteers for any oil spill.  The 54 volunteers in our dataset form the sample, a subset from all volunteers who clean wildlife following an oil spill.  Our population of interest is comprised of all volunteers who clean wildlife following an oil spill. 

:::{.callout-note}
When identifying the population of interest for a research objective, be specific!  Suppose you are trying to estimate the average height of trees.  Are you really interested in _all_ trees?  Or, are you interested in Maple trees within the city limits of Terre Haute, Indiana?  This distinction impacts how we collect the data.
:::
...
:::{#def-response}
## Response
The primary variable of interest within a study.  This is the variable you would either like to explain or predict.
:::
...
:::{.callout-tip}
## Big Idea
Research questions need to focus on characterizing the _distribution_ of the variable.
:::

In this case, the response is a categorical variable; describing the distribution of such a variable is equivalent to describing how individuals are divided among the possible groups.  With a finite number of observations, we could present the number of observations, the __frequency__, within each group.  For example, of the 54 volunteers, 15 experienced adverse symptoms and 39 did not.  This works well within the sample; however, as the population becomes infinitely large (all volunteers cleaning wildlife following an oil spill), reporting frequencies is no longer appropriate.  As a result, even in small samples, it is common to report the fraction of observations, the __relative frequency__, falling within each group in addition to the frequency; this helps convey information about the distribution of this variable.  That is, the relative frequencies give us a sense of which values of the variable are more or less common in the sample.
...
Numeric quantities, like the proportion, that summarize the distribution of a variable within the population are known as __parameters__.

:::{#def-parameter}
## Parameter
Numeric quantity that summarizes the distribution of a variable within the _population_ of interest.  Parameters are generally denoted by Greek letters in statistical formulas.
:::

While the value of a _variable_ may vary across subjects in the population, the _parameter_ is a single fixed constant that summarizes the distribution of the variable across that population.  For example, the grade received on an exam varies from one student to another in a class; but, the _average exam grade_ is a fixed number that summarizes the exam grade across the entire class.  Well-posed questions are then those that acknowledge variability and focus on the _distribution_ of the variable instead of the value of the variable for an individual subject.  And, since parameters characterize distributions, well-posed questions are formed by focusing on parameters.  The second step in constructing well-posed questions is then to identify the parameter of interest.  

Most scientific questions generally fall into one of two categories:
...
:::{.callout-important}
## Fundamental Idea I
A research objective can often be framed in terms of a parameter that characterizes the distribution of a variable across the population.
:::

We now have a way of describing a well-posed question, a question which can be addressed using data.  Well-posed questions are about the population and are framed in terms of a parameter.  Such questions guide the analysis.  


## Framing the Question
In engineering and scientific applications, many questions fall under the second category of __hypothesis testing__, which is a form of model comparison where data is collected to help the researcher choose between two competing theories for the parameter of interest.  In this section, we consider the terminology surrounding specifying such questions.
...
We can now collect data and determine if it is _consistent_ with the null hypothesis (a statement similar to "not guilty") or if the data provides _evidence_ against the null hypothesis and therefore _evidence_ in favor of the alternative hypothesis (a statement similar to "guilty").

:::{.callout-warning}
## Consistent vs. Evidence
The term "consistent" and "reasonable" will be used interchangeably throughout the text; however, these terms differ substantially from the term "evidence."  The data is said to be consistent with a statement if the data is aligned with that statement.  We have evidence against a statement ("disproving" a statement) when the data is not aligned with that statement.  We have evidence for a statement ("proving" the statement) when the data is aligned with that statement, _and_ the data is not aligned with the opposite of the statement.  We will see this developed more in Chapters [-@sec-samplingdistns] and [-@sec-nulldistns].
:::

Hypotheses are often written in a more mathematical structure in which a Greek letter is used to represent the parameter of interest.  For example, we might write
...
:::{#def-null-value}
## Null Value
The value associated with the equality component of the null hypothesis; it forms the threshold or boundary between the hypotheses.  Not all questions of interest require a null value be specified.
:::

:::{.callout-tip}
## Big Idea
Hypothesis testing is a form of statistical inference where we quantify the evidence _against_ a working theory (captured by the null hypothesis).  We essentially argue that the data supports the alternative if it is not consistent with the working theory.
:::
...
In this research objective, we are not comparing two competing theories.  We are interested in estimation, not hypothesis testing and thus there is no corresponding null and alternative hypothesis.
