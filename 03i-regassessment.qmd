# Assessing the Modeling Conditions {#sec-regassessment}
...
```{r}
#| label: regassessment-diagnostics
fit.greece.slr <- lm(BD02 ~ Magnitude, data = greece.df)
fit.greece.slr.diag <- augment(fit.greece.slr)
```

```{r}
#| label: fig-regassessment-mean0
#| fig-cap: Plot of the residuals vs. the predicted values for a model predicting bracketed duration as a function of the magnitude of an earthquake.
#| fig-alt: Scatterplot of the residuals and predicted values; two wide vertical regions are shaded in green.  These "windows" help to examine trends.
ggplot(data = fit.greece.slr.diag) +
  geom_point(mapping = aes(x = .fitted,
                           y = .resid)) +
  geom_ribbon(data = tibble(x = c(4, 4.75), 
                                ymax = c(6, 6), 
                                ymin = c(-5, -5)),
              mapping = aes(x = x, ymax = ymax, ymin = ymin),
              fill = "green", alpha = 0.2) +
  geom_ribbon(data = tibble(x = c(10.25, 11),
                                ymax = c(20, 20),
                                ymin = c(-11, -11)),
              mapping = aes(x = x, ymax = ymax, ymin = ymin),
              fill = "green", alpha = 0.2) +
...
```{r}
#| label: fig-regassessment-independence-violations
#| fig-cap: Examples of trends in a time-series plot of the residuals.  Such trends indicate the data is not consistent with the condition that the errors are independent of one another.
#| fig-alt: Two line plots are shown of the residuals over time; in the first, the line plot is increasing left to right, and in the second, the line plot becomes more variable from left to right.
set.seed(201708)
...
```{r}
#| label: fig-regassessment-independence-reasonable
#| fig-cap: Example of a time-series plot of residuals which shows no trends in location or spread.  This is consistent with what we would expect if the condition of independence among errors were satisfied.
#| fig-alt: Line plot of residuals over time; there are no trends in the location or spread.
set.seed(201708)
...
```{r}
#| label: fig-regassessment-independence
#| fig-cap: Time-series plot of the residuals for a model predicting bracketed duration as a function of the magnitude of an earthquake.
#| fig-alt: Line plot of residuals against time; no clear trends in the location or spread are visible.
ggplot(data = fit.greece.slr.diag,
...
```{r}
#| label: fig-regassessment-normal
#| fig-cap: Probability plot of the residuals for a model predicting bracketed duration as a function of the magnitude of an earthquake.
#| fig-alt: A scatterplot of residuals against the theoretical quantiles we would expect if the residuals were drawn from a Normal distribution.
ggplot(data = fit.greece.slr.diag,
...
```{r}
#| label: fig-regassessment-normal-comparison
#| fig-cap: Two Normal probability plots of the residuals for two hypothetical datasets.  The trend away from a straight line in Panel A suggests assuming the errors follow a Normal distribution would be unreasonable.  However, the reasonably straight line in Panel B suggests the data is consistent with the errors following a Normal distribution.
#| fig-alt: Two scatterplots of the sample quantiles against the theoretical quantiles are given; in Panel A, there is clear curvature, and in Panel B, the points form a relatively straight line.
set.seed(201709)

plot.dat <- tibble(
  panel = rep(c('Panel A', 'Panel B'), each = 100),
  resids = c(rexp(100), rnorm(100))
) |>
  group_by(panel) |>
  mutate(
