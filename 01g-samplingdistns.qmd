# Assessing the Evidence (Quantifying the Variability in Estimates) {#sec-samplingdistns}
...
```{r}
#| label: fig-samplingdistns-sampling-distribution
#| fig-cap: Illustration of repeatedly sampling from a population.
#| fig-alt: A blue blob represents the population; several arrows extend from the population to green blobs, each representing a unique sample.  An area to the right shows that each sample is summarized by a statistic.
...
```{r}
#| label: fig-samplingdistns-dice-example
#| fig-cap: Potential sample from rolling a die 20 times.
#| fig-alt: Twenty dice are lined up; there is no pattern to the values shown on the dice.
...
```{r}
#| label: fig-samplingdistns-dice-dotplot
#| fig-cap: Sampling distribution for the proportion of 20 rolls of a die which result in a 1.  The distribution is based on repeating the sampling process 500 times.
#| fig-alt: Dotplot of the proportion of 20 dice rolls resulting in a 1 from 500 simulations; the peak of the plot is at 0.167.
...
```{r}
#| label: fig-samplingdistns-dice-histogram
#| fig-cap: Sampling distribution for the proportion of 20 rolls of a die which result in a 1.  The distribution is based on repeating the sampling process 50000 times.
#| fig-alt: Histogram of the proportion of 20 dice rolls resulting in a 1 from 50000 simulations; the peak of the plot is at 0.167.
...
ggplot(data = dice.df,
       mapping = aes(x = Props)) +
  geom_histogram(aes(y = after_stat(count)/50000), binwidth = 0.05,
...
```{r}
#| label: fig-samplingdistns-dice-histogram2
#| fig-cap: Sampling distribution for the proportion of 60 rolls of a die which result in a 1.  The distribution is based on repeating the sampling process 50000 times.
#| fig-alt: Histogram of the proportion of 60 dice rolls resulting in a 1 from 50000 simulations; the peak of the plot is at 0.167.
...
```{r}
#| label: fig-samplingdistns-deepwater-histogram
#| fig-cap: Sampling distribution for the proportion of volunteers assigned to wildlife who develop adverse symptoms based on a sample of 54 volunteers.
#| fig-alt: Histogram of the proportion of 54 volunteers experiencing adverse symptoms created from a model of the sampling distribution.
...
```{r}
#| label: fig-samplingdistns-deepwater-ci
#| fig-cap: Construction of a confidence interval via bootstrapping for the proportion of volunteers assigned to wildlife who develop adverse symptoms based on a sample of 54 volunteers.
#| fig-alt: Histogram of the proportion of 54 volunteers who report adverse symtoms; two vertical red lines represent the lower and upper bounds of a 95% confidence interval.
...
ggplot(data = deepwater.boot.df,
  ...
  geom_histogram(aes(y = after_stat(count)/50000), binwidth = 1/54,
                 colour = "black", fill = "grey75") +
  geom_vline(xintercept = ci95[1], colour = "red", linewidth = 1.5) +
  geom_vline(xintercept = ci95[2], colour = "red", linewidth = 1.5) +
...
```{r}
#| label: fig-samplingdistns-ci-process
#| fig-cap: Performance of the confidence interval process (for the sample mean) across repeated samples.  Our idea of "confidence" requires repeated sampling.  The process of constructing a 95% confidence interval captures the parameter in 95% of repeated samples; it is not the specific interval, but the process that works 95% of the time.
#| fig-alt: Density plot representing population, several histograms representing different samples, and several histograms representing different models for sampling distributions.
#| fig-asp: 1.67
...
popn <- tibble(
  ...
  annotate('label', x = 1, y = 0, label = 'mu', parse = TRUE, color = 'blue') +
  coord_cartesian(xlim = c(0, 6)) +
  labs(x = 'Value of Variable',
       y = NULL,
       subtitle = 'Distribution of the Population') +
  theme(
...
psamp1 <- samp1 |>
  ggplot() +
  aes(x = x) +
  geom_histogram(fill = 'grey75', color = 'black', binwidth = 0.2) +
  geom_vline(xintercept = mean(samp1$x), color = 'darkgreen', linetype = 2) +
  annotate('label', x = mean(samp1$x), y = 0, label = 'bar(x)', parse = TRUE, color = 'darkgreen') +
  coord_cartesian(xlim = c(0, 6)) +
  labs(x = 'Value of Variable',
       y = NULL,
       subtitle = 'Distribution of the Sample') +
  theme(
    axis.ticks = element_blank(),
    axis.text = element_blank()
  )
...
psamp2 <- samp2 |>
  ...
  aes(x = x) +
  geom_histogram(fill = 'grey75', color = 'black', binwidth = 0.2) +
  geom_vline(xintercept = mean(samp2$x), color = 'darkgreen', linetype = 2) +
  annotate('label', x = mean(samp2$x), y = 0, label = 'bar(x)', parse = TRUE, color = 'darkgreen') +
  coord_cartesian(xlim = c(0, 6)) +
  labs(x = 'Value of Variable',
       y = NULL) +
  theme(
...
psamp3 <- samp3 |>
  ...
  aes(x = x) +
  geom_histogram(fill = 'grey75', color = 'black', binwidth = 0.2) +
  geom_vline(xintercept = mean(samp3$x), color = 'darkgreen', linetype = 2) +
  annotate('label', x = mean(samp3$x), y = 0, label = 'bar(x)', parse = TRUE, color = 'darkgreen') +
  coord_cartesian(xlim = c(0, 6)) +
  labs(x = 'Value of Variable',
       y = NULL) +
  theme(
...
psd1 <- sd1 |>
  ggplot() +
  aes(x = x) +
  geom_histogram(fill = 'grey75', color = 'black', binwidth = 0.05) +
  geom_vline(xintercept = 1, color = 'blue', linetype = 2) +
  annotate('label', x = 1, y = 0, label = 'mu', parse = TRUE, color = 'blue') +
  geom_vline(xintercept = mean(samp1$x), color = 'darkgreen', linetype = 2) +
  annotate('label', x = mean(samp1$x), y = 0, label = 'bar(x)', parse = TRUE, color = 'darkgreen') +
  geom_vline(xintercept = 1.1 - 2/5, colour = "red", linewidth = 1.5) +
  geom_vline(xintercept = 1.1 + 2/5, colour = "red", linewidth = 1.5) +
  annotate("segment", x = 1.1 - 2/5, y = 10, 
           xend = 1.1 + 2/5, yend = 10, 
           arrow = arrow(ends = "both"),
           colour = "red", linewidth = 1.5) +
  annotate("label", x = 1.1, y = 10, label = '95% CI') +
  labs(x = 'Value of Sample Mean',
       y = NULL,
       subtitle = 'Model of the Sampling Distribution') +
...
psd2 <- sd2 |>
  ...
  geom_vline(xintercept = mean(samp2$x), color = 'darkgreen', linetype = 2) +
  annotate('label', x = mean(samp2$x), y = 0, label = 'bar(x)', parse = TRUE, color = 'darkgreen') +
  geom_vline(xintercept = 2.2 - 2/5, colour = "red", linewidth = 1.5) +
  geom_vline(xintercept = 2.2 + 2/5, colour = "red", linewidth = 1.5) +
  annotate("segment", x = 2.2 - 2/5, y = 10, 
           xend = 2.2 + 2/5, yend = 10, 
           arrow = arrow(ends = "both"),
           colour = "red", linewidth = 1.5) +
  annotate("label", x = 2.2, y = 10, label = '95% CI') +
  labs(x = 'Value of Sample Mean',
       y = NULL) +
  theme(axis.ticks = element_blank(),
        axis.text = element_blank())
...
psd3 <- sd3 |>
  ...
  geom_vline(xintercept = mean(samp3$x), color = 'darkgreen', linetype = 2) +
  annotate('label', x = mean(samp3$x), y = 0, label = 'bar(x)', parse = TRUE, color = 'darkgreen') +
  geom_vline(xintercept = 0.7 - 2/5, colour = "red", linewidth = 1.5) +
  geom_vline(xintercept = 0.7 + 2/5, colour = "red", linewidth = 1.5) +
  annotate("segment", x = 0.7 - 2/5, y = 10, 
           xend = 0.7 + 2/5, yend = 10, 
           arrow = arrow(ends = "both"),
           colour = "red", linewidth = 1.5) +
  annotate("label", x = 0.7, y = 10, label = '95% CI') +
  labs(x = 'Value of Sample Mean',
       y = NULL) +
  theme(axis.ticks = element_blank(),
        axis.text = element_blank())
