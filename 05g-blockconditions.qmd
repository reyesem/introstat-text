# Conditions on the Error Term of the Repeated Measures ANOVA Model {#sec-blockconditions}
...
The third condition that is typically placed on the distribution of the errors is that the variability of the errors is the same for all combination of the predictors.  Again, we encountered this condition in @sec-regconditions and @sec-anovaconditions.  As our model includes both the group comparisons of interest as well as the block terms, violating this condition happens if the response is more precise for one group/block combination than another.
...
```{r}
#| label: blockconditions-fit
fit.yogurt <- lm(Taste ~ Type + `Participant ID` - 1, data = yogurt.df)
```

```{r}
#| label: tbl-blockconditions-summary
#| tbl-cap: Estimated parameters in a model for the taste ratings of yogurt from three vendors using data from a randomized complete block design with 9 blocks.
...
fit.yogurt |>
  ...
  mutate(
    Term = str_remove_all(Term, 'Type'),
    Term = str_replace_all(Term, '`Participant ID`', 'Participant ')
  ) |>
  mykable(digits = 3)
```
